...
public class GraphAnalysis {
    ...
    public static void extractANDsaveCCandKCore (PrintWriter cc_pw, PrintWriter kcore_pw, String[] cluster_terms, WeightedUndirectedGraph graph) throws InterruptedException, IOException {       
        ...
        for (Set<Integer> component : connected_components) {   
            component.stream().forEach((node_id) -> {
                cc_pw.println(cluster_terms[node_id]+" ");
            });
            ...
            for (int node_id : kcore.seq) {
                kcore_pw.println(cluster_terms[node_id] + " ");
            }            
        }       
...
//    public static void extractKCoreAndConnectedComponent(double threshold, int k) throws IOException, ParseException, Exception {
//
